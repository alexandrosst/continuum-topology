...
4. **GeoIP**: the server image already bundles a small, free DB-IP City Lite database, downloaded once when the image was *built* - a genuinely air-gapped runtime cluster needs nothing further for this, since the file already sits in the image you mirrored in step 1. Set `geoip.path` (and a volume) only to use a different `.mmdb` instead; nothing else is downloaded at runtime either way. `geoip.asnPath` (which network an address belongs to, alongside its location) is entirely your own file, same volume, no network call, same as before. The UI loads no third-party assets (its Content-Security-Policy is `'self'` only), so all of this works offline. Leave `geoip.publicIpService` unset here: it is the one feature in this chart that makes its own outbound call *at runtime* to a third party (an IP-echo service you choose, to estimate a same-network agent's location - see the values reference below), which an air-gapped install by definition cannot reach; it fails soft (that one estimate is just never available) rather than blocking startup, but there is no point configuring it.
...
| `geoip.publicIpService` | `""` | needs `geoip.path`; estimate an agent whose connecting address can't be located at all from this server's own public IP (one outbound call, cached) |
| `geoip.asnPath` | `""` | needs `geoip.path`; second, offline `.mmdb` in ASN shape - adds which network an address belongs to (AS number, organisation) alongside its location |
...
**An agent shows no location suggestion, or the wrong one, although this isn't a home/single-node network.** The server suggests a location from the address it saw the agent connect from; if that address isn't the agent's own, geoip has nothing real to look up. Check first that nothing set `agent.service.externalTrafficPolicy` back to `Cluster` (it defaults to `Local` precisely to avoid the most common cause, a Kubernetes Service SNATing the connection when it lands on a different node than the pod). If a reverse proxy or TCP passthrough hop still sits in front after that (see the agent port note above), configure it to send a PROXY protocol header and set `agent.behindProxy=true` so the server trusts it.

**An agent genuinely shares a network with the server (a home lab, a single-site on-prem install) and you want a location anyway.** There is no location to *discover* there - the agent's own address is private because it never crosses the public internet on its way to the server, not because of a bug. `geoip.publicIpService` (needs `geoip.path`) is an explicit, opt-in way to show one anyway: the server makes its own "what is my public IP" call to a URL you choose (`https://api.ipify.org` is a well known one), and estimates that agent's location from this server's own public address instead - correct exactly when the two do share a network, which is the case this is for. The result is always marked `estimated` (in the UI, a `(estimated)` tag; in the API, `connectingGeo.estimated: true`) and never rises above low confidence, including when it happens to agree with a region label. It's off by default because it's the one thing in this server that calls an outside party of your choosing rather than an offline database; see `geoip.path`'s note above for what to do instead in an air-gapped install, and `networkPolicy.egress.geoipPublicIpCIDRs` if `networkPolicy.enabled`.

**You'd like more than city/country for a connecting address - which network it's actually on.** `geoip.asnPath` (needs `geoip.path`) is a second, offline `.mmdb` in ASN shape rather than city/country shape (a free download: MaxMind's GeoLite2-ASN, or DB-IP's ASN Lite). Wherever `geoip.path` places an address - including a `geoip.publicIpService` estimate - the AS number and the organisation that announces it (e.g. `15169` / "Google LLC") are added alongside it. It's often a steadier signal than city or country: a VPN or a cloud provider's egress can move the place shown, but not who is actually carrying the traffic. Same mount, same fail-closed behavior as `geoip.path`; nothing about it calls out to the network.
