...
4. **GeoIP** (optional): supply the `.mmdb` yourself (`geoip.path` and a volume); nothing is downloaded. `geoip.asnPath` (which network an address belongs to, alongside its location) is the same story - a second offline file, same volume, no network call. The UI loads no third-party assets (its Content-Security-Policy is `'self'` only), so both work offline. Leave `geoip.publicIpService` unset here: it is the one feature in this chart that makes its own outbound call to a third party (an IP-echo service you choose, to estimate a same-network agent's location - see the values reference below), which an air-gapped install by definition cannot reach; it fails soft (that one estimate is just never available) rather than blocking startup, but there is no point configuring it.
...
| `geoip.path` | `""` | offline `.mmdb` for location suggestions; mount the file with `extraVolumes` |
