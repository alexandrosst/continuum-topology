...
const FIT: Record<Verdict, { label: string; tone: string; Icon: typeof Check; help: string }> = {
  fits: { label: 'Fits', tone: 'border-emerald-400/40 bg-emerald-400/10 text-emerald-300', Icon: Check, help: 'Every check holds, even at the pessimistic end of every figure that is not exact.' },
  cantTell: { label: 'Can’t tell', tone: 'border-dashed border-amber-400/60 bg-amber-400/10 text-amber-300', Icon: CircleHelp, help: 'Something that decides it is not known, or too uncertain to certify. It is not a yes and it is not a no.' },
  doesNotFit: { label: 'Does not fit', tone: 'border-red-400/40 bg-red-400/10 text-red-300', Icon: X, help: 'Something rules it out, even at the optimistic end of every figure that is not exact.' },
}
...
export function Hedge({ level, inputs, verdict }: { level: 'low' | 'none' | 'medium' | 'high'; inputs: { class: Class }[]; verdict?: Verdict }) {
  ...
  return (
    <p className="flex items-start gap-2 rounded-lg border border-dashed border-amber-400/50 bg-amber-400/5 px-3 py-2 text-sm text-amber-200" role="note" data-testid="hedge" data-level={level}>
...
function FactLine({ f, now }: { f: FactRow; now: number }) {
  ...
  return (
    <li className="flex flex-wrap items-baseline gap-x-2 gap-y-1 py-2 text-xs" data-testid="fact" data-attribute={f.attribute} data-confidence={f.confidence} data-known={f.value !== null}>
      ...
      <span className={clsx('tabular-nums', f.value === null ? 'italic text-amber-300' : 'text-white')} data-testid="fact-value">{value}</span>
      ...
      {age && <span className={clsx('text-nb-500', f.aged && 'text-amber-300')}>{f.aged ? 'not confirmed for ' : 'confirmed '}{age}{f.aged ? '' : ' ago'}</span>}
...
export function Why({ facts, wouldChange, fixes = [], sensitivity = [], now, label = 'Why', defaultOpen, children }: {
  ...
  return (
    <div className="mt-2 text-xs" data-testid="why">
      <button type="button" onClick={() => setOpen((o) => !o)} className="inline-flex items-center gap-1 text-nb-400 hover:text-white" aria-expanded={open} aria-controls={id} data-testid="why-toggle">
