...
- **`:8080`, the admin port.** The web UI and the JSON API it's built on. Plain HTTP inside the pod, meant to be put behind a Gateway that terminates TLS (or given its own certificate directly with `admin.tls`). If you don't provide one, the chart generates a self-signed certificate for it automatically rather than refusing to install — real HTTPS for a trial or LAN-reachable install with nothing else to set up.
