...
If you're plugging in your own policy — this is the part built for that — two things make it a stable integration point rather than something to reverse-engineer from traffic captures. First, the whole request and response shape is a formal, machine-readable contract:
[`backend/docs/decider-webhook.openapi.yaml`](https://github.com/alexandrosst/continuum-topology/blob/main/backend/docs/decider-webhook.openapi.yaml)
(OpenAPI 3.1; `schema` only ever changes for a break, and new fields are always additive, so code generated against it today keeps working). Second, setting a **shared secret** next to the address (Settings → Deciders) makes the server sign every request it sends you — HMAC-SHA256 over the timestamp and body, the same convention Stripe and GitHub webhooks use — so your decider can tell a real call from this server apart from a guessed address or a replayed one; the spec above documents the exact scheme, and a small reference implementation you can port line for line is right next to the server code that signs the request, in [`internal/server/decide_sign.go`](https://github.com/alexandrosst/continuum-topology/blob/main/backend/internal/server/decide_sign.go).
